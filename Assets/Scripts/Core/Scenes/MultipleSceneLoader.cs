...
namespace twinkocat.Core.Scenes
{
    ...
    {
        private static readonly LoadingScreen LoadingScreen =
            UnityExtensions.LoadAndInstantiate<LoadingScreen>("LoadingScreen")
            ?? throw new NullReferenceException("Need to create a LoadingScreenPrefab in ResourceFolder");

        public static async Task LoadSceneWithTryCatchBlock(List<SceneData> scenesToLoadList,
            bool loadScreenActive = true, bool reloadDubScene = false)
        {
            try
            {
        ...
        public static async Task LoadScenes(List<SceneData> scenesToLoadList, bool loadScreenActive = true,
            bool reloadDubScene = false)
        {
            ...
            var activeScene = SceneManager.GetSceneByName(scenesToLoadList
                .FirstOrDefault(sceneData => sceneData.sceneType is SceneType.ActiveScene)?.Name);
        ...
        private static async Task UnloadScenes(List<SceneData> sceneToLoad)
        {
            ...
            for (var i = sceneCount - 1; i > 0; i--)
            {
                ...
                if (handleGroup.Any(operationHandle =>
                        operationHandle.IsValid() && operationHandle.Result.Scene.name == sceneAt.name)) continue;
